...
export function evalTemplate(hass, state, custom_variables, func, template_literals = false) {
	if (template_literals) func = `return \`${func}\`;`;
...
export function getTemplateOrValue(hass, state, custom_variables, value, _callback) {
	...
	if (_callback) value = _callback(value);
...
export function provideHass(element) {
	if (document.querySelector('home-assistant')) return document.querySelector('home-assistant').provideHass(element);
...
export function lovelace_view() {
	...
	root = (root && root.querySelector('hui-view')) || root;
	return root;
}
...
export function honeycomb_menu_templates() {
	...
	if (window.honeycomb_menu_templates) return window.honeycomb_menu_templates;
	...
	if (!lconfig.honeycomb_menu_templates) return null;
...
function _errorElement(error, origConfig) {
	...
	customElements.whenDefined('hui-error-card').then(() => {
		...
		newel.setConfig(cfg);
		if (el.parentElement) el.parentElement.replaceChild(newel, el);
	});
	return el;
}
...
export function createCard(config) {
	if (!config || typeof config !== 'object' || !config.type) return _errorElement(`No card type configured`, config);

	let tag = config.type;
	if (tag.startsWith('custom:')) tag = tag.substr('custom:'.length);
	else tag = `hui-${tag}-card`;

	if (customElements.get(tag)) return _createElement(tag, config);
